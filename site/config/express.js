...
module.exports.http = {
  ...
  customMiddleware: function(app){
    app.get("/assets/css/all.gz.css", function(req, res, next) {
      res.header('Content-Encoding', 'gzip');
      res.header('Transfer-Encoding', 'gzip');
      next();
    });

    app.get("/scripts/lib_js_all.gz.js", function(req, res, next) {
      res.header('Content-Encoding', 'gzip');
      res.header('Transfer-Encoding', 'gzip');
      next();
    });

    app.get("/scripts/logic_js_all.gz.js", function(req, res, next) {
      res.header('Content-Encoding', 'gzip');
      res.header('Transfer-Encoding', 'gzip');
      next();
    });
