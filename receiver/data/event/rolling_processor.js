var EventEmitter = require('events').EventEmitter,
  ...
  RollingMachineState = require('../../config/event_config/state_config').erelay2_state,
...
var RollingProcessor = function () {
  EventProcessor.call(this);
};

util.inherits(RollingProcessor, EventProcessor);
module.exports = new RollingProcessor();

RollingProcessor.prototype.handleData = function (oldState, newState, attachedInfo) {
  var myEvent = {};
  ...
  if (oldState.online_state !== newState.online_state) {
    switch (newState.online_state) {
      case RollingMachineState.online_state.online:
        ...
        myEvent.detail = eventUtil.erelay2Data(RollingMachineState.online_state.online, attachedInfo);
        generateEvent(myEvent);
        break;
      case RollingMachineState.online_state.offline:
        MyRedis.myRedis.sodc();
        myEvent.event_name = EVENTCONFIG.event_name_config.DEVICE_OFFLINE;
        myEvent.event_type = EVENTCONFIG.event_type_config.NOTICE;
        myEvent.event_level = EVENTCONFIG.event_level_config.NOTICE.NOTICE_LEVEL_1;
        myEvent.detail = eventUtil.erelay2Data(RollingMachineState.online_state.offline, attachedInfo);
        generateEvent(myEvent);
        break;
      default:
        break;
