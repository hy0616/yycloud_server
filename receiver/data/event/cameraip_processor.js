var EventEmitter = require('events').EventEmitter,
  ...
  CameraipState = require('../../config/event_config/state_config').cameraip_state,
...
var CameraIpProcessor = function () {
  EventProcessor.call(this);
};

util.inherits(CameraIpProcessor, EventProcessor);
module.exports = new CameraIpProcessor();

CameraIpProcessor.prototype.handleData = function (oldState, newState, attachedInfo) {
  var myEvent = {};
  ...
  if (oldState.online_state !== newState.online_state) {
    switch (newState.online_state) {
      case CameraipState.online_state.online:
        ...
        myEvent.detail = eventUtil.cameraipData(CameraipState.online_state.online, attachedInfo);
        generateEvent(myEvent);
        break;
      case CameraipState.online_state.offline:
        MyRedis.myRedis.sodc();
        myEvent.event_name = EVENTCONFIG.event_name_config.DEVICE_OFFLINE;
        myEvent.event_type = EVENTCONFIG.event_type_config.NOTICE;
        myEvent.event_level = EVENTCONFIG.event_level_config.NOTICE.NOTICE_LEVEL_1;
        myEvent.detail = eventUtil.cameraipData(CameraipState.online_state.offline, attachedInfo);
        generateEvent(myEvent);
        break;
      default:
        break;
